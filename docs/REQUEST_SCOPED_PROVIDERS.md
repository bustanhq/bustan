...
`HttpRequest` is the framework's own request type and is the one to reach for: it is the same object under every adapter, so a provider written against it is not tied to the transport the application happens to serve on. A provider may take the serving transport's own request object instead - `starlette.requests.Request` under the Starlette adapter - and `request.native_request` reaches that object from the neutral one. Both count as request state for the rules below, but the two spellings are not equally safe: an annotation naming a transport's request type is read as a role rather than as that class, which [PLATFORM_INTEGRATION.md](PLATFORM_INTEGRATION.md#a-native-annotation-names-a-role-not-a-class) explains.
