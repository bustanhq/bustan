...
class ParseIntPipe(Pipe):
    ...
    async def transform(self, value: object, context: ParameterContext) -> int:
        try:
...
class ParseFloatPipe(Pipe):
    ...
    async def transform(self, value: object, context: ParameterContext) -> float:
        try:
...
class ParseBoolPipe(Pipe):
    ...
    async def transform(self, value: object, context: ParameterContext) -> bool:
        normalized = str(value).strip().lower()
        if normalized in _TRUTHY:
            return True
        if normalized in _FALSY:
            return False
        raise BadRequestException("Validation failed (boolean expected)", field=context.name)


class ParseUUIDPipe(Pipe):
    """Convert a parameter value into a UUID instance."""

    async def transform(self, value: object, context: ParameterContext) -> UUID:
        try:
            return UUID(str(value))
        except (TypeError, ValueError) as exc:
            raise BadRequestException("Validation failed (UUID expected)", field=context.name) from exc
...
class ParseArrayPipe(Pipe):
    ...
    async def transform(self, value: object, context: ParameterContext) -> list[str]:
...
class ParseEnumPipe(Pipe):
    ...
    async def transform(self, value: object, context: ParameterContext) -> Enum:
...
class ValidationPipe(Pipe):
    ...
    async def transform(self, value: object, context: ParameterContext) -> object:
        ...
        try:
            ...
            raise BadRequestException(str(exc), field=context.name) from exc
