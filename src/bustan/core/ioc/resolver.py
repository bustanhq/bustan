...
class Resolver:
        ...
        *,
        ...
        try:
            ...
            if binding.scope in (ProviderScope.SINGLETON, ProviderScope.DURABLE):
                ...
                lock, store = self._shared_instance_slot(
    ...
    def _check_for_cycle(self, token: object, declaring_module: ModuleKey) -> None:
        ...
        current_stack = self.resolution_stack.get()
        if any(
            frame.token == token and frame.module == declaring_module for frame in current_stack
        ):
            cycle_path = " -> ".join(
            ...
            if binding.scope in (ProviderScope.SINGLETON, ProviderScope.DURABLE):
                ...
                async with async_lock:
                    ...
                    return self._cache_instance(
                        binding, binding_key, declaring_module, token, instance
                    )
    ...
    def _plan_constructor_parameters(
        ...
        for parameter in signature.parameters.values():
            ...
            elif dependency.optional and not self._is_dependency_available(
                dependency.token, module_key
            ):
                planned_parameters.append(
