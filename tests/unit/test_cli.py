...
from star.cli import main


def test_cli_new_scaffolds_a_project_with_expected_files(tmp_path: Path, capsys) -> None:
    exit_code = main(["new", "hello-star", "--directory", str(tmp_path)])

    assert exit_code == 0
    project_directory = tmp_path / "hello-star"
    ...
    assert (project_directory / "src" / "hello_star" / "app.py").exists()
    ...
    app_content = (project_directory / "src" / "hello_star" / "app.py").read_text(encoding="utf-8")
    stdout = capsys.readouterr().out

    assert "star.cli:main" not in pyproject_content
    assert '"uvicorn>=0.30.0,<1.0.0"' in pyproject_content
    assert 'uvicorn.run("hello_star.app:app", reload=True)' in app_content
    assert "Created Star application" in stdout
...
def test_cli_rejects_non_empty_target_directory(tmp_path: Path, capsys) -> None:
    ...
    exit_code = main(["new", "existing-app", "--directory", str(tmp_path)])
