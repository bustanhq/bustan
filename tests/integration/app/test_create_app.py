...
def test_create_app_returns_a_starlette_application_with_module_graph_state() -> None:
    ...
    controller_instance = cast(
        Any, application._container.instantiate_class(UserController, module=AppModule)
    )

    # Check internal module graph state for the root module
    assert application._container.module_graph.root_module is AppModule
    assert controller_instance.user_service is application._container.resolve(
        UserService,
        module=AppModule,
    )
...
def test_create_app_registers_compiled_adapter_routes_in_deterministic_order() -> None:
    ...
    assert [compiled_route.path for compiled_route in adapter.registered_routes] == [
        "/zeta",
        "/alpha",
    ]
    assert all(
        isinstance(compiled_route, CompiledAdapterRoute)
        for compiled_route in adapter.registered_routes
    )
